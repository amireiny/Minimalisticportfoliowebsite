...
export function Navigation() {
  ...
  useEffect(() => {
    const detectColors = () => {
      ...
      // 1. Skip detection if mobile menu is open (usually covers screen anyway)
      ...
      try {
        ...
        // Center-y sampling point
        const centerY = navTop + navHeight / 2;
        const sigX = 100;
        const textX = window.innerWidth - 200;

        // 2. DISABLE pointer events so document.elementFromPoint "sees through" the nav
        // We don't touch opacity anymore, so the nav stays visually present.
        navRef.current.style.pointerEvents = "none";

        // 3. Perform the sampling synchronously
        const sigElement = document.elementFromPoint(
          sigX,
          centerY,
        );
        const textElement = document.elementFromPoint(
          textX,
          centerY,
        );

        // 4. RESTORE pointer events immediately
        navRef.current.style.pointerEvents = "auto";

        // 5. Process Signature Color
        const sigBg = getBackgroundColor(sigElement);
        const sigLuminance = getLuminance(sigBg);
        const newSigColor =
          sigLuminance < 128 ? "#ffffff" : "#000000";

        // Only update state if color actually changed to avoid unnecessary re-renders
        setSignatureColor((prev) =>
          prev !== newSigColor ? newSigColor : prev,
        );

        // 6. Process Text Color
        const textBg = getBackgroundColor(textElement);
        const textLuminance = getLuminance(textBg);
        const newTextColor =
          textLuminance < 128 ? "#ffffff" : "#000000";

        setTextColor((prev) =>
          prev !== newTextColor ? newTextColor : prev,
        );
      } catch (error) {
  ...
  const signatureHoverColor =
    signatureColor === "#ffffff" ? "#d4d4d4" : "#8b8b8b";
  const textHoverColor =
    textColor === "#ffffff" ? "#d4d4d4" : "#8b8b8b";
